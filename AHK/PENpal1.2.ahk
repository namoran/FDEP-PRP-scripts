...
while WinActive("SmarTerm for Windows")
    {
        If (A_Index < 500)
	ToolTip, Press Ctrl+Shift+V to Paste in PEN.
        Else if (A_Index < 501){
	ToolTip
	Break
	}
...
{
SplashTextOn, 330, 530,HELP,
(
Press`nCtrl+Shift+V`nto Paste in PEN 
`n`nCtrl+Shift+I`nto insert Inspection 
`n`nCtrl+Alt+C`nto insert CO 
`n`nCtrl+Alt+X`nto reject with today's date 
`n`nCtrl+Alt+V`nto approve with today's date 
)
WinMove, HELP, , 10, 70
}
Return
...
Winclose, ahk_class AutoHotkeyGUI
Gui, New,+AlwaysOnTop
Gui, Add, Button, Default x1 y19 w70 h30 , ENTER

...
DetectionSummary = []
EDD = []
#The variables below will hold sets (aka unique items), they will be used to construct standard format tables
WaterTableColumns = set()
SoilTableColumns = set()
WellSet = set()
WaterSampleEvents = set()
...
with fileinput.input(sys.argv[1:]) as fileset:
	...
	for count, row in enumerate(reader):
                ...
                if row['Matrix'] == 'W':        # Everything within this if is for water samples
                        WaterTableColumns.add(row['Parameter'])
                        WellSet.add(row['TestSite_Name'])
                        WaterSampleEvents.add(row['Sample_Date'])
                        if   GCTLs.get(row['Parameter'].lower()) != None:        
                                if   row['Units'].lower() == 'ug/l' and float(row['Result']) > float(GCTLs.get(row['Parameter'].lower())):
...
print(WaterTableColumns)
print(WellSet)
print(WaterSampleEvents)
